...
export default function WeeklyAINewsletter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    submissionData.append('list', '68GRsk325uhvP08mY8Mxmw');
    submissionData.append('subform', 'yes');
    // Honeypot field for spam prevention, should be empty
    submissionData.append('hp', '');

    try {
      const response = await fetch("https://emails.lla.in/subscribe", {
        method: "POST",
        body: submissionData,
        // The 'Content-Type' header is automatically set by the browser
        // to 'multipart/form-data' when using FormData.
      });

      if (response.ok) {
        // Redirect to a confirmation page on successful submission
